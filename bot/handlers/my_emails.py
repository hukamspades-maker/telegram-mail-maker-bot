...
async def handle_list_aliases(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not aliases:
        await query.edit_message_text(
            ...
            "Click ⚡ <b>Quick Random Mail</b> or ✏️ <b>Custom Prefix Mail</b> to create one.",
            reply_markup=get_back_button(),
            parse_mode="HTML"
        )
        return

    text = f"📬 <b>Your Active Emails on hukam.bond ({len(aliases)}):</b>\n\n"
    buttons = []
    ...
    buttons.append([InlineKeyboardButton("🔑 Login / Restore Another Email", callback_data="mail_login_key")])
...
async def handle_delete_alias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await query.answer("Deleted address!", show_alert=True)
